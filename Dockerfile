...
RUN apt-get update \
 ...
 && find /etc/systemd/system \
        ...
        -exec rm \{} \; \
 && curl -sL https://www.postgresql.org/media/keys/ACCC4CF8.asc | gpg --dearmor | tee /etc/apt/trusted.gpg.d/apt.postgresql.org.gpg >/dev/null \
...
RUN apt-get -y --no-install-recommends install /ubnt-archive-keyring_*_arm64.deb \
 ...
 && sed -i -e 's/WatchdogSec/#WatchdogSec/g' /lib/systemd/system/unifi-core.service \
 && sed -i -e 's/Type=notify/Type=simple/g' /lib/systemd/system/unifi-core.service
